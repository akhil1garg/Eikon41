...
{
    public class CompanyOverviewAppWebApp : GCPAppBase
    {
        String title; IWebDriver driver;
        //const string DIRECT_PATH = "/Apps/news-monitor#savedinstance=true";
        const string DIRECT_PATH = "/Apps/Corp?s=IBM&st=RIC&app=true#/Summary";
        // const string REDIRECT_PATH = "/Explorer/EVzFXMMzSPOTxSTREAMING.aspx?s=EUR%3d&st=RIC";
        IWebElement locator;

        #region Constructor for WhiteArk framework
        public CompanyOverviewAppWebApp(IWebDriver webDriver, string windowHandle)
            : base(webDriver, windowHandle)
        {

        }

        public CompanyOverviewAppWebApp(IWebDriver webDriver)
            : base(webDriver)
        {

        }
        public CompanyOverviewAppWebApp(string title, IWebDriver webDriver)
        ...
        public CompanyOverviewAppWebApp(IEikonDriver eikonDriver, string windowTitle)
            : base(eikonDriver)
        {
            Url = eikonDriver.RootUrl + DIRECT_PATH;
        }

        public CompanyOverviewAppWebApp(IEikonDriver eikonDriver)
