...
{
    public class TopNewsAppWebApp : GCPAppBase
    {
        String title; IWebDriver driver;
       
        const string DIRECT_PATH = "/Apps/TopNews?group=SP_GROUP_001&page=SP_PAGE_001#/tn/SP_PAGE_001";
        
        IWebElement locator;

        #region Constructor for WhiteArk framework
        public TopNewsAppWebApp(IWebDriver webDriver, string windowHandle)
            : base(webDriver, windowHandle)
        {

        }

        public TopNewsAppWebApp(IWebDriver webDriver)
            : base(webDriver)
        {

        }
        public TopNewsAppWebApp(string title, IWebDriver webDriver)
        ...
        public TopNewsAppWebApp(IEikonDriver eikonDriver, string windowTitle)
            : base(eikonDriver)
        {
            Url = eikonDriver.RootUrl + DIRECT_PATH;
        }

        public TopNewsAppWebApp(IEikonDriver eikonDriver)
