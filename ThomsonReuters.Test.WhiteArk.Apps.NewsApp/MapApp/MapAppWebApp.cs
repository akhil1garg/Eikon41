...
{
    public class MapAppWebApp : GCPAppBase
    {
        String title; IWebDriver driver;
        //const string DIRECT_PATH = "/Apps/news-monitor#savedinstance=true";
        const string DIRECT_PATH = "/Apps/AppLibrary#/filtered?searchString=interactive%20map";
        // const string REDIRECT_PATH = "/Explorer/EVzFXMMzSPOTxSTREAMING.aspx?s=EUR%3d&st=RIC";
        IWebElement locator;

        #region Constructor for WhiteArk framework
        public MapAppWebApp(IWebDriver webDriver, string windowHandle)
            : base(webDriver, windowHandle)
        {

        }

        public MapAppWebApp(IWebDriver webDriver)
            : base(webDriver)
        {

        }
        public MapAppWebApp(string title, IWebDriver webDriver)
        ...
        public MapAppWebApp(IEikonDriver eikonDriver, string windowTitle)
            : base(eikonDriver)
        {
            Url = eikonDriver.RootUrl + DIRECT_PATH;
        }

        public MapAppWebApp(IEikonDriver eikonDriver)
